...
int main(void)
{
...
#ifdef USE_VIDEO
	CvCapture *input_video = cvCreateFileCapture("vid1.mp4");
#else
	...
	int width = video_size.width / 2;
	int height = video_size.height / 2;
	...
	IplImage *half_frame = cvCreateImage(cvSize(width / 2, height / 2), IPL_DEPTH_8U, 3);
	...
	while (key_pressed != 27) {
		...
		cvResize(frame, frame1, CV_INTER_LINEAR);

		cvPyrDown(frame1, half_frame, CV_GAUSSIAN_5x5); // Reduce the image by 2	 
		//cvCvtColor(temp_frame, grey, CV_BGR2GRAY); // convert to grayscale

		// process vehicles
		vehicleDetection(frame1, cascade, haarStorage);
		drawVehicles(frame1);
		cvShowImage("Half-frame", frame1);
		cvMoveWindow("Half-frame", 0, 0);
